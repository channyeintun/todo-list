import styles from './icons.module.css';

export const ChevDownSolid = () => {
      return (
            <svg width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M6.50423 0.420898L3.99998 2.92515L1.49573 0.420898L0.670898 1.24573L3.99998 4.57482L7.32907 1.24573L6.50423 0.420898Z" fill="black" />
            </svg>
      )
}

interface IconPropType {
      onClick?: (value: any) => void;
}

export const CheckedSolid = ({ onClick = () => { } }: IconPropType) => {

      return (
            <div onClick={onClick} className={`${styles.checked} ${styles.checkbox}`}>
                  <TickSolid />
            </div>
      )

}

export const UnCheckedSolid = ({ onClick = () => { } }: IconPropType) => {

      return (
            <div onClick={onClick} className={`${styles.unchecked} ${styles.checkbox}`}>

            </div>
      )

}

const TickSolid = () => {
      return (
            <svg width="11" height="9" viewBox="0 0 11 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4.08339 6.04014L1.75085 3.7076L0.749268 4.70918L4.08339 8.04331L10.9592 1.16751L9.9576 0.165932L4.08339 6.04014Z" fill="white" />
            </svg>
      )
}

export const OptionsDots = () => {
      return (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M7.19941 11.9999C7.19941 12.6364 6.94656 13.2468 6.49647 13.6969C6.04638 14.147 5.43593 14.3999 4.79941 14.3999C4.16289 14.3999 3.55245 14.147 3.10236 13.6969C2.65227 13.2468 2.39941 12.6364 2.39941 11.9999C2.39941 11.3633 2.65227 10.7529 3.10236 10.3028C3.55245 9.85272 4.16289 9.59986 4.79941 9.59986C5.43593 9.59986 6.04638 9.85272 6.49647 10.3028C6.94656 10.7529 7.19941 11.3633 7.19941 11.9999ZM14.3994 11.9999C14.3994 12.6364 14.1466 13.2468 13.6965 13.6969C13.2464 14.147 12.6359 14.3999 11.9994 14.3999C11.3629 14.3999 10.7524 14.147 10.3024 13.6969C9.85227 13.2468 9.59941 12.6364 9.59941 11.9999C9.59941 11.3633 9.85227 10.7529 10.3024 10.3028C10.7524 9.85272 11.3629 9.59986 11.9994 9.59986C12.6359 9.59986 13.2464 9.85272 13.6965 10.3028C14.1466 10.7529 14.3994 11.3633 14.3994 11.9999V11.9999ZM19.1994 14.3999C19.8359 14.3999 20.4464 14.147 20.8965 13.6969C21.3466 13.2468 21.5994 12.6364 21.5994 11.9999C21.5994 11.3633 21.3466 10.7529 20.8965 10.3028C20.4464 9.85272 19.8359 9.59986 19.1994 9.59986C18.5629 9.59986 17.9524 9.85272 17.5024 10.3028C17.0523 10.7529 16.7994 11.3633 16.7994 11.9999C16.7994 12.6364 17.0523 13.2468 17.5024 13.6969C17.9524 14.147 18.5629 14.3999 19.1994 14.3999Z" fill="#9796A8" />
            </svg>
      )
}